...
function updateNetworkStatus() {
    ...
    if (connection) {
        const effectiveType = connection.effectiveType || '4g';
        networkType.textContent = effectiveType.toUpperCase();
...
function updateStats() {
    if (!art || !art.video || !currentHls) return;
    ...
    // 更新码率
    if (currentHls.levels && currentHls.currentLevel >= 0) {
        const currentLevel = currentHls.levels[currentHls.currentLevel];
        const bitrate = (currentLevel.bitrate / 1000000).toFixed(2);
        document.getElementById('statBitrate').textContent = `${bitrate} Mbps`;
    }

    // 更新下载速度
    const now = Date.now();
    const timeDiff = (now - lastStatsTime) / 1000;
    if (currentHls.stats && timeDiff > 0) {
        const bytesLoaded = currentHls.stats.loaded || 0;
        const bytesDiff = bytesLoaded - lastBytesLoaded;
        const speed = (bytesDiff / timeDiff / 1024).toFixed(0);
        document.getElementById('statSpeed').textContent = `${speed} KB/s`;
    ...
    if (buffered.length > 0) {
        ...
        document.getElementById('statBuffer').textContent = `${bufferLength}s (${bufferStallCount}次)`;
    }

    // 更新丢帧信息
    if (video.getVideoPlaybackQuality) {
        const quality = video.getVideoPlaybackQuality();
        document.getElementById('statDropped').textContent = quality.droppedVideoFrames;
    }
...
// 监听缓冲事件
if (typeof art !== 'undefined' && art && art.video) {
    art.video.addEventListener('waiting', function() {
        bufferStallCount++;
    });
}